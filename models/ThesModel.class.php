...
class ThesModel extends ModelManager{
    ...
    public function getBest(){
        $req = "SELECT Thes.id_the, titre_the,sousTitre_the,desc_the,cdc_the,publish,image_the,min(QuantiThes.prix) as prix FROM Thes INNER JOIN QuantiThes ON QuantiThes.id_the = Thes.id_the WHERE Thes.id_the = ( SELECT MAX(Thes.id_the) FROM Thes) GROUP BY Thes.id_the  "
                 ;
